...
import type { TradingXbertAnalysis, Market, Style, JournalEntry } from "@/lib/tradingTypes";
...
Generated by TradingXbert.com 🤖✨
  ...
  const handleShare = async () => {
    ...
    const text = `Check out my ${analysis.signal} trade analysis with ${analysis.confidence}% confidence! 📊`;
  ...
  const handleSaveToJournal = () => {
    // Save to localStorage journal immediately
    if (typeof window !== 'undefined' && analysis) {
      const preview = localStorage.getItem('currentChartPreview');
      ...
      if (preview) {
        const entry: JournalEntry = {
          ...
          imageData: preview,
          analysis,
          market: (localStorage.getItem('currentMarket') || 'Crypto') as Market,
          style: (localStorage.getItem('currentStyle') || 'Day Trade') as Style,
        };
        
        saveToJournal(entry).then(() => {
          setSaved(true);
          setTimeout(() => setSaved(false), 2000);
          alert('✅ Saved to your journal!');
        }).catch((err) => {
          console.error('Failed to save journal entry:', err);
          alert('⚠️ Failed to save to journal. Storage may be full.');
        });
      } else {
        alert('⚠️ No chart data available to save');
      }
    }
  };
  ...
  return (
    <div className="w-full max-w-7xl mx-auto animate-in fade-in-0 zoom-in-95 duration-500">
      ...
      {(analysis.signal === "LONG" || analysis.signal === "SHORT") && (
        <div className="mb-6 relative bg-gradient-to-br from-[#FFD700]/20 via-[#FFA500]/10 to-transparent backdrop-blur-xl rounded-2xl border-2 border-[#FFD700] p-6 shadow-2xl">
          ...
          <div className="mt-4 p-4 bg-black/20 rounded-xl border border-white/10">
            <div className="text-lg font-bold text-white mb-2">✅ Action Plan:</div>
            <ul className="space-y-1 text-sm text-neutral-300">
              <li>• {analysis.signal === "LONG" ? "Wait for price to reach entry zone (support)" : "Wait for price to reach entry zone (resistance)"}</li>
              <li>• Set your Stop Loss {analysis.signal === "LONG" ? "below support" : "above resistance"} to limit risk</li>
              <li>• Set your Take Profit at the target level</li>
              <li>• Use proper position sizing based on your risk tolerance ({analysis.riskLevel} risk setup)</li>
              <li>• Monitor for confirmation signals before entering</li>
            </ul>
          </div>
        </div>
      )}
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-6">
        {/* Trend & Structure */}
        <AnalysisCard title="Trend & Structure" icon="📊" iconGradient="from-blue-400 to-cyan-400" compact>
          <p className="text-neutral-200 leading-relaxed text-sm">{analysis.trendSummary}</p>
        </AnalysisCard>

        {/* Pattern Summary */}
        <AnalysisCard title="Pattern Summary" icon="🔍" iconGradient="from-purple-400 to-pink-400" compact>
          <p className="text-neutral-200 leading-relaxed text-sm">{analysis.patternSummary}</p>
        </AnalysisCard>

        {/* Key Levels */}
        <AnalysisCard title="Key Levels" icon="🎯" iconGradient="from-red-400 to-orange-400" compact>
      ...
      <div className="space-y-4 mb-6">
        {/* Style Notes */}
        <AnalysisCard title="Trading Strategy" icon="💡" iconGradient="from-emerald-400 to-green-400" fullWidth compact>
          <p className="text-neutral-200 leading-relaxed text-sm">{analysis.styleNotes}</p>
        </AnalysisCard>

        {/* Emotion Check */}
        {analysis.emotionSummary && (
          <AnalysisCard title="Psychology Check" icon="🧠" iconGradient="from-indigo-400 to-purple-400" fullWidth compact>
          ...
          <div className="text-neutral-200 leading-relaxed text-sm space-y-2">
            {(typeof analysis.teachingTips === 'string' ? analysis.teachingTips : String(analysis.teachingTips || '')).split('\n').filter(tip => tip.trim()).map((tip, i) => (
              <div key={i} className="flex items-start gap-2 p-2 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                <span className="text-[#FFD700] text-lg flex-shrink-0">💎</span>
        ...
        {analysis.confidenceBreakdown && (
          <AnalysisCard title="Confidence Breakdown" icon="📈" iconGradient="from-teal-400 to-cyan-400" fullWidth compact>
